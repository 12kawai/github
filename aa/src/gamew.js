// gamew.js
let alertContent = ''; // alertContent変数を適切に宣言しました

const BLACK = new Player(RANDOM);   // 人が操作
const WHITE = new Player(HUMAN);  // コンピュータが操作(ランダム)
  ...
  loop() {
    ...
    switch (this.state) {
      ...
      case GAME_END:
        ...
        alertContent = winnerMessage;

        // POSTリクエストを送信
        const xhr = new XMLHttpRequest();
        xhr.open('POST', 'oserow.php', true);
        xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        xhr.send('result_message=' + winnerMessage);        
        break;
      default:
        break;
    }
